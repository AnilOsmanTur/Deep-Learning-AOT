  ...
  def train(self,best=False, early_stop=False):
    ...
    for t in range(num_iterations):
      ...
      if epoch_end:
        ...
        mean_loss = self.check_accuracy(features, captions, num_samples=None, batch_size=int(num_train *0.2))
      ...
      if self.verbose and t % self.print_every == 0:
        if self.epoch < 1:
            mean_loss = 999.9
        print ('(Iteration %d / %d) loss: %f val_mean_loss: %f best val loss: %f' % (
               t + 1, num_iterations, self.loss_history[-1], mean_loss, best_loss))

      if patiance > 3 and early_stop:
        print ('Early stopping loss: %f val_mean_loss: %f best val loss: %f' % (
